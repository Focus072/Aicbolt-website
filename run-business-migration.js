...
async function runBusinessMigration() {
  console.log('🚀 Starting Business Control Center migration...');
  ...
  try {
    ...
    console.log('📝 Running business tables migration...');
    ...
    if (teamsResult.length > 0) {
      ...
      const existingClients = await sql`
        SELECT id FROM clients WHERE team_id = ${team.id}
      `;
      ...
      if (existingClients.length === 0) {
        ...
        console.log('   - 3 Clients added');
        console.log('   - 3 Revenue entries');
        console.log('   - 4 Expense entries');
        console.log('   - 3 AI Projects');
    ...
    console.log('');
    console.log('🎉 Business Control Center migration completed successfully!');
